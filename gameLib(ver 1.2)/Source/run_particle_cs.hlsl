...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	//if (startIndex * createFlag <= index && startIndex * createFlag + 10 + 90 * moveType * createFlag > index)
	//{
	...
	p.life -= elapsdTime;
	//徐々に大きくする
	p.scale += lerp(float3(0.1, 0.1, 0.1), float3(0, 0, 0), step(1.4f, p.scale.x)) * elapsdTime * 60;
	//徐々に不透明にしていく
