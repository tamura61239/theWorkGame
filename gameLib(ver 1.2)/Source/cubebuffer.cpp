...
CubeBuffer::CubeBuffer(ID3D11Device* device, int width, int height, bool msaaFlag, int subsamples, DXGI_FORMAT renderTargetTexture2dFormat, int arraySize, DXGI_FORMAT depthStencilTexture2dFormat, bool renderTargetShaderResourceViewFlag, bool depthStencilShaderResourceViewFlag, bool mipsFlag)
{
	...
	if (renderTargetTexture2dFormat != DXGI_FORMAT_UNKNOWN)
	{
		...
		texture2d_desc.ArraySize = arraySize;
		...
		render_target_view_desc.Texture2DArray.ArraySize = arraySize;
	...
	if (depthStencilTexture2dFormat != DXGI_FORMAT_UNKNOWN)
	{
		...
		depth_stencil_view_desc.Texture2DArray.ArraySize = arraySize;
		depth_stencil_view_desc.Texture2DArray.FirstArraySlice = 0;
		depth_stencil_view_desc.Texture2DArray.MipSlice = 0;
		depth_stencil_view_desc.Flags = D3D11_RESOURCE_MISC_TEXTURECUBE;
		hr = device->CreateDepthStencilView(texture2d.Get(), nullptr, mDepthStencilView.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

		//if (depthStencilShaderResourceViewFlag)
		//{
		//	D3D11_SHADER_RESOURCE_VIEW_DESC shader_resource_view_desc = {};
		//	shader_resource_view_desc.Format = DXGI_FORMAT_D32_FLOAT;
		//	shader_resource_view_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURECUBE;
		//	shader_resource_view_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURECUBE;
		//	shader_resource_view_desc.TextureCube.MostDetailedMip = 0;
